...
def main():
    ...
    local_output_prefix = "{}.prelim".format(output_prefix)

    ## run virus integration site analysis, generate report
    # generates: output_prefix + ".full.tsv" and output_prefix + ".abridged.tsv"
    cmd = " ".join(
        [
    ...
    prelim_full_chim_events_file = "{}.abridged.tsv".format(local_output_prefix)  # generated by above

    pipeliner.run()

    prelim_filtered_chim_events_file = "{}.abridged.filtered.tsv".format(local_output_prefix)
    filter_prelim_filtered_chim_events(prelim_full_chim_events_file, prelim_filtered_chim_events_file, min_reads)


    ## generate genome wide insertion site abundance plot
    prelim_genome_wide_abundance_plot = prelim_filtered_chim_events_file + ".genome_plot.png"

    cmd = " ".join(
        [
            os.path.join(UTILDIR, "make_VIF_genome_abundance_plot.Rscript"),
            "--vif_report {}".format(prelim_filtered_chim_events_file),
    ...
    num_prelim_chim_events = count_num_candidates(prelim_filtered_chim_events_file)
    ...
    cmd = " ".join(
        [
            ...
            "--chim_events {}".format(prelim_filtered_chim_events_file),
    ...
    cmd = " ".join(
        [
            ...
            "--prelim_counts {}".format(prelim_filtered_chim_events_file),
...
def filter_prelim_filtered_chim_events(input_file, output_file, value):
    import pandas as pd
    df = pd.read_csv(input_file, sep='\t')
    df = df[df['total'] >= value]
