...
def main():
    ...
    prelim_chim_events_file = "{}.abridged.tsv".format(local_output_prefix)  # generated by above

    pipeliner.run()

    if min_reads > 0:
        prelim_chim_events_file = filter_events(prelim_chim_events_file, "{}.filtered.abridged.tsv".format(local_output_prefix), min_reads)
...
def filter_events(prelim_chim_events_file, output_file, min_reads):
    import pandas as pd
    df = pd.read_csv(prelim_chim_events_file, sep='\t')
